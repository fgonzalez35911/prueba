...
try {
    ...
    if (is_numeric($last_check_timestamp) && $last_check_timestamp > 0) { 
        ...
        $sql_new = "
            ...
            AND leida = 0 
            -- AND UNIX_TIMESTAMP(fecha_creacion) > :last_time_seconds <-- FILTRO COMENTADO PARA EVITAR EL BUG DE SINCRONIZACIÓN
            ORDER BY fecha_creacion ASC";

        $stmt_new = $pdo->prepare($sql_new);
        $stmt_new->bindParam(':id_user', $id_usuario, PDO::PARAM_INT);
        // $stmt_new->bindParam(':last_time_seconds', $last_time_seconds, PDO::PARAM_INT); <-- BINDING COMENTADO
        $stmt_new->execute();
